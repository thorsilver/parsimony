...
\newcommand{\bbstatenumcomma}{$BB($7,918), }
\newcommand{\bbstatenumperiod}{$BB($7,918). }
...
For vertices $x$ and $y$, $x \le_{rlex} y$ if and only if $x = y$ or $x_{|x|-i} < y_{|y|-i}$ where $i$ is least such that $x_{|x|-i} \not= y_{|y|-i}$.\footnotemark (The $\le_{rlex}$ operation creates a lexicographic ordering over the vertices, weighting the last and largest elements of those vertices most heavily.)
\footnotetext{Friedman recommended in private communication that we use the $\le_{rlex}$ comparator to compare vertices, instead of comparing their maximum elements as described in his manuscript.~\cite{friedmancomm}}
